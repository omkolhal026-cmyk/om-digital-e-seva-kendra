...
async function startServer() {
  ...
  async function checkUserPermission(req: express.Request, requiredPerm: 'canRegister' | 'canRenew' | 'canClaim' | 'canExport' | 'adminOnly') {
    ...
    const user = allUsers.find((u) => u.id === userId || u.username.toLowerCase() === username.toLowerCase());
  ...
  app.post('/api/renewals', async (req, res) => {
    try {
      ...
      addLog(
        ...
        'operator',
        'RENEWAL_CREATE',
        `Processed renewal ${newRenewal.id} for MH No: ${newRenewal.mhNumber} (${newRenewal.workerName}). Status: ${newRenewal.status}`
      );
