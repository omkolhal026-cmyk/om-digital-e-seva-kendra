...
import { BotVerifierModule } from './components/BotVerifierModule';
import { PendingVerificationModule } from './components/PendingVerificationModule';
import { MaterialDistributionModule } from './components/MaterialDistributionModule';
import { WhatsappGroupPendingModule } from './components/WhatsappGroupPendingModule';
import { IwbmsCheckerModule } from './components/IwbmsCheckerModule';
import { SubAgentManagementModule } from './components/SubAgentManagementModule';
import { SubAgentEntriesModule } from './components/SubAgentEntriesModule';
import { ClaimPaymentsModule } from './components/ClaimPaymentsModule';
...
export default function App() {
  ...
  const [subAgentEntriesFilter, setSubAgentEntriesFilter] = useState<string>('all');
  ...
  const handleAddRegistrationsBulk = async (records: Omit<WorkerRegistration, 'id'>[]) => {
    try {
      const token = getStoredAccessToken();
      ...
      if (res.ok && data.data) {
        ...
        throw new Error(data.error || 'Failed to bulk import registrations');
      }
    } catch (err) {
      console.error(err);
      throw err;
  ...
  const handleAddRenewal = async (newRenData: Omit<WorkerRenewal, 'id'>) => {
    try {
      ...
      const created = await res.json();
      if (res.ok) {
        if (created.secondRenewal) {
          setRenewals((prev) => [created.secondRenewal, created, ...prev]);
        } else {
  ...
  const handleAddRenewalsBulk = async (records: Omit<WorkerRenewal, 'id'>[]) => {
    try {
      const token = getStoredAccessToken();
      const res = await fetch('/api/renewals/bulk', {
        method: 'POST',
        headers: {
          ...getAuthHeaders(),
          'Content-Type': 'application/json',
      ...
      if (res.ok && data.data) {
        ...
        throw new Error(data.error || 'Failed to bulk import renewals');
      }
    } catch (err) {
      console.error(err);
      throw err;
  ...
  const handleDeleteClaim = async (id: string) => {
    try {
      const res = await fetch(`/api/claims/${id}?operator=${currentUser?.username}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
      if (res.ok) {
  ...
  const handleDeleteUser = async (id: string) => {
    try {
      ...
      if (res.ok && data.success) {
        ...
        const errorMsg = data.error || 'ऑपरेटर हटवण्यात अडचण आली (Failed to delete operator)';
        alert(errorMsg);
      }
    } catch (err: any) {
      console.error('Failed to delete operator:', err);
      alert('ऑपरेटर हटवण्यात अडचण आली: ' + (err?.message || 'नेटवर्क त्रुटी'));
    }
  };
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {activeTab === 'registration' && (
              <RegistrationModule
                ...
                onAddRegistrationsBulk={handleAddRegistrationsBulk}
            ...
            {activeTab === 'material-distribution' && (
              <MaterialDistributionModule currentUser={currentUser} />
            )}

            {activeTab === 'whatsapp-group-pending' && (
              <WhatsappGroupPendingModule currentUser={currentUser} />
            )}
            ...
            {activeTab === 'renewal' && (
              <RenewalModule
                ...
                onAddRenewalsBulk={handleAddRenewalsBulk}
            ...
            {activeTab === 'sub-agent-management' && currentUser.role === 'admin' && (
              <SubAgentManagementModule
                currentUser={currentUser}
                onOpenPrintSlip={(type, data) => setPrintModalInfo({ type, data })}
                onViewSubAgentEntries={(subAgentId) => {
                  setSubAgentEntriesFilter(subAgentId || 'all');
                  setActiveTab('sub-agent-entries');
                }}
              />
            )}

            {activeTab === 'sub-agent-entries' && (
              <SubAgentEntriesModule
                currentUser={currentUser}
                initialSubAgentFilter={subAgentEntriesFilter}
                onOpenPrintSlip={(type, data) => setPrintModalInfo({ type, data })}
              />
            )}
