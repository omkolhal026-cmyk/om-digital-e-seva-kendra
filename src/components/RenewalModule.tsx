...
import { exportToCSV, formatDate, normalizeDateToYMD } from '../utils/exportUtils';
...
export const RenewalModule: React.FC<RenewalModuleProps> = ({
  ...
  const handleMhLookup = (value: string) => {
    ...
    if (clean.length >= 8) {
      // Check for duplicate renewal entry
      const existingRenewal = renewals.find(
        (r) =>
          r.mhNumber.trim().toUpperCase().replace(/[\s-]/g, '') === clean &&
          r.id !== editingId
      );
      if (existingRenewal) {
        setValidationErrors((prev) => ({
          ...prev,
          mhNumber: `या MH नंबर (${existingRenewal.mhNumber}) साठी रिन्युअल नोंद आधीच अस्तित्वात आहे! (Double entry not allowed)`,
        }));
      } else {
        setValidationErrors((prev) => {
          const copy = { ...prev };
          delete copy.mhNumber;
          return copy;
        });
      }
      ...
      const matched = registrations.find(
        (r) =>
          r.mhNumber.toUpperCase().replace(/[\s-]/g, '') === clean ||
  ...
  const validate = (): boolean => {
    ...
    if (!formData.mhNumber.trim()) {
      ...
      // Check for double entry / duplicate renewal in renewals list
      const existing = renewals.find(
        (r) =>
          r.mhNumber.trim().toUpperCase().replace(/[\s-]/g, '') === mhClean &&
          r.id !== editingId
      );
      if (existing) {
        errors.mhNumber = `या MH नंबर (${existing.mhNumber}) साठी रिन्युअल नोंद आधीच अस्तित्वात आहे! (Duplicate Renewal Entry)`;
      }
    }
  ...
  const handleExcelImportRenewals = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const row of rawRows) {
        ...
        const renewalDate = normalizeDateToYMD(rawRenDate) || getTodayDate();
      ...
      <div className="p-4 rounded-2xl bg-white border border-slate-200/90 shadow-xs space-y-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-100 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-600">नूतनीकरण तारीख (Renewal Date):</span>
            <input
              type="date"
              value={fromDate}
          ...
          <div className="flex items-center gap-3">
            ...
            {(searchTerm || talukaFilter || statusFilter || operatorFilter || fromDate || toDate) && (
              <button
                onClick={() => {
                  setSearchTerm('');
                  setTalukaFilter('');
        ...
        <div ref={renTableContainerRef} className="overflow-x-auto max-h-[65vh] overflow-y-auto scroll-smooth">
          <table className="w-full text-left text-xs text-slate-800 border-collapse">
            <thead className="bg-slate-100/95 backdrop-blur-xs text-slate-700 font-bold uppercase tracking-wider text-[10px] border-b border-slate-200 sticky top-0 z-20 shadow-2xs">
              <tr>
                <th className="p-3 w-12 text-center sticky left-0 bg-slate-100 z-30 border-r border-slate-200/80">Sr. No.</th>
                <th className="p-3 sticky left-12 bg-slate-100 z-30 shadow-xs border-r border-slate-200/80 min-w-[150px]">Full Name</th>
                <th className="p-3 min-w-[140px]">MH Reg Number</th>
                <th className="p-3 min-w-[120px]">Mobile Number</th>
                <th className="p-3 min-w-[110px]">Verification Date</th>
                <th className="p-3 min-w-[110px]">Renewal Date</th>
          ...
          <div className="bg-white border border-slate-200/90 rounded-3xl max-w-2xl w-full p-6 shadow-2xl relative text-slate-900 my-8">
            ...
            <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* 5. Renewal Date (Auto Today - Editable) */}
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="block text-xs font-bold text-slate-800">
                      5. Renewal Date / नूतनीकरण तारीख
                    </label>
                    <button
                      type="button"
                  ...
                  <input
                    ...
                    title="नूतनीकरण तारीख (आवश्यकतेनुसार बदलू शकता)"
                  />
              ...
              <div className="pt-3 border-t border-slate-200">
                ...
                {formData.status === 'Active' && (
                  <div className="mt-2.5 p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-[11px] text-emerald-900 flex items-center gap-2">
                    ...
                    <span>
                      Renewal status will be saved as <b>Active</b> in MySQL database.
                    </span>
                  </div>
                )}
                {formData.status === 'Pending' && (
                  <div className="mt-2.5 p-3 rounded-xl bg-amber-50 border border-amber-200 text-[11px] text-amber-900 flex items-center gap-2">
                    <span className="text-sm">🟡</span>
                    <span>
                      Renewal status will be saved as <b>Pending</b> in MySQL database.
                    </span>
                  </div>
                )}
              </div>
