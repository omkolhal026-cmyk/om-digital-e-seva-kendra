...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  React.useEffect(() => {
    fetch('/api/dashboard/stats', {
      ...
      .catch(() => {});
  }, [currentUser.id, currentUser.role, registrations.length, renewals.length, claims.length]);

  // Memoized client calculations (used as instant base & fallback)
  const clientDailyReg = React.useMemo(() => registrations.filter((r) => r.registrationDate === todayStr).length, [registrations, todayStr]);
  const clientDailyRen = React.useMemo(() => renewals.filter((r) => r.renewalDate === todayStr).length, [renewals, todayStr]);
  const clientDailyClm = React.useMemo(() => claims.filter((c) => c.claimDate === todayStr).length, [claims, todayStr]);
  const clientPendingReg = React.useMemo(() => registrations.filter((r) => r.status === 'Pending Verification' || r.status === 'Pending').length, [registrations]);
  const clientPendingClm = React.useMemo(() => claims.filter((c) => c.status === 'Submitted' || c.status === 'Under Scrutiny').length, [claims]);
  const clientTotalDisbursed = React.useMemo(
    () => claims.filter((c) => c.status === 'Disbursed' || c.status === 'Approved').reduce((sum, c) => sum + (c.totalAmount || 0), 0),
    [claims]
  );

  const clientTalukaCounts = React.useMemo(() => {
  ...
  const totalReg = serverStats ? serverStats.totalReg : registrations.length;
  const totalRen = serverStats ? serverStats.totalRen : renewals.length;
  const totalClm = serverStats ? serverStats.totalClm : claims.length;

  const dailyReg = serverStats ? serverStats.dailyReg : clientDailyReg;
  const dailyRen = serverStats ? serverStats.dailyRen : clientDailyRen;
  const dailyClm = serverStats ? serverStats.dailyClm : clientDailyClm;
  const dailyWorkCount = serverStats ? serverStats.dailyWorkCount : dailyReg + dailyRen + dailyClm;

  const pendingReg = serverStats ? serverStats.pendingReg : clientPendingReg;
  const pendingClm = serverStats ? serverStats.pendingClm : clientPendingClm;
  const pendingWorkCount = serverStats ? serverStats.pendingWorkCount : pendingReg + pendingClm;
  const totalDisbursed = serverStats ? serverStats.totalDisbursed : clientTotalDisbursed;

  const todayIncome = serverStats?.todayIncome || { total: 0, cash: 0, online: 0 };
  const talukaCounts: Record<string, number> = (serverStats && serverStats.talukaCounts && Object.keys(serverStats.talukaCounts).length > 0) ? serverStats.talukaCounts : clientTalukaCounts;
  ...
  return (
    <div className="space-y-6 pb-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="py-2.5 px-4 rounded-xl bg-white/10 hover:bg-white/20 text-white border border-white/30 font-bold text-xs flex items-center gap-2 transition-all active:scale-95"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <div
          onClick={() => onSelectTab('pending-verification')}
          className="cursor-pointer group p-5 rounded-2xl bg-gradient-to-b from-red-50 to-white border border-red-200 hover:border-red-500 transition-all shadow-xs hover:shadow-md relative overflow-hidden"
        >
          <div className="absolute top-0 right-0 w-24 h-24 bg-red-100/50 rounded-full blur-xl group-hover:bg-red-200/60 transition-all" />
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-red-800 uppercase tracking-wider flex items-center gap-1">
              🔴 Pending Verif.
            </span>
            <div className="p-2 rounded-xl bg-red-100 text-red-700 border border-red-300 group-hover:scale-110 transition-transform">
              <AlertCircle className="w-5 h-5 text-red-600 animate-pulse" />
            </div>
          </div>
          <div className="text-2xl md:text-3xl font-black text-red-900 tracking-tight">
            {pendingWorkCount}
          </div>
          <div className="mt-2 text-[11px] text-red-700 font-bold flex items-center justify-between">
            <span>Reg ({pendingReg}) + Claims ({pendingClm})</span>
        ...
        <div
          ...
          className="p-5 rounded-2xl bg-white border border-slate-200/90 shadow-xs relative overflow-hidden cursor-pointer hover:border-emerald-300 transition-colors group"
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Daily Work & Income</span>
            <div className="p-2.5 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200/60 group-hover:bg-emerald-100 transition-colors">
          ...
          <div className="flex items-baseline justify-between">
            <div>
              ...
              <div className="text-[11px] text-slate-500 font-medium">Work Done Today</div>
            </div>
            <div className="text-right">
              <div className="text-xl font-bold text-emerald-700 font-mono">₹{todayIncome.total.toLocaleString('en-IN')}</div>
              <div className="text-[10px] text-emerald-600 font-semibold">Today's Income</div>
            </div>
          </div>
          <div className="mt-3 pt-2 border-t border-slate-100 text-[10px] text-slate-500 flex justify-between items-center font-medium">
            <span>Cash: <strong className="text-amber-700">₹{todayIncome.cash.toLocaleString('en-IN')}</strong> | Online: <strong className="text-emerald-700">₹{todayIncome.online.toLocaleString('en-IN')}</strong></span>
            <span className="text-blue-600 font-semibold flex items-center gap-0.5 group-hover:underline">
              Daily Report <ChevronRight className="w-3 h-3" />
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 p-6 rounded-2xl bg-white border border-slate-200/90 shadow-xs space-y-4">
          ...
          <div className="space-y-3 pt-2">
            {Object.entries(talukaCounts).map(([taluka, count]) => {
              ...
              const percent = Math.round((countNum / maxTalukaCount) * 100);
              return (
                <div key={taluka} className="space-y-1">
                  <div className="flex justify-between text-xs text-slate-700 font-semibold">
                    <span>Taluka {taluka}</span>
                    <span className="text-blue-700 font-bold">{countNum} Workers</span>
                  </div>
