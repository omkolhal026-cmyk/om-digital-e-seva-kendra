  ...
  const handleConfirmNotEligible = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/material-distributions/${notEligibleModalItem.record.id}/status`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({
            materialType: notEligibleModalItem.materialType,
      ...
      setSuccessMsg(
        `Worker ${notEligibleModalItem.record.workerName} marked as NOT ELIGIBLE.`
      );
      setTimeout(() => setSuccessMsg(''), 4000);
