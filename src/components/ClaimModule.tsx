...
import { Award, Clock, Sparkles, AlertCircle, RefreshCw, Trash2 } from 'lucide-react';
...
export const ClaimModule: React.FC<ClaimModuleProps> = ({ claims = [], onResetClaims }) => {
  return (
    <div className="space-y-6 min-h-[70vh] flex flex-col items-center justify-center">
      <div className="bg-white border border-slate-200/90 rounded-3xl p-8 md:p-12 shadow-sm text-center max-w-xl w-full flex flex-col items-center">
        ...
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200 mb-4">
          ...
          <span>Feature Under Development (Claim Management Status: Disabled)</span>
        </span>
        ...
        <p className="text-sm text-slate-600 mb-6 leading-relaxed max-w-md">
          The Welfare Scheme Claim Management module is currently paused. Total current active claims entries in database: <span className="font-bold text-slate-900">{claims.length}</span>.
        </p>

        {onResetClaims && (
          <button
            onClick={onResetClaims}
            className="mb-6 flex items-center gap-2 px-5 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-xl text-xs font-bold transition-all shadow-xs hover:shadow-md cursor-pointer active:scale-95"
          >
            <Trash2 className="w-4 h-4 text-rose-600" />
            <span>क्लेम नोंदी रीसेट करा (Reset Claims to 0)</span>
          </button>
        )}
