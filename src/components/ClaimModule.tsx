...
export const ClaimModule: React.FC<ClaimModuleProps> = ({
  ...
  const filteredClaims = claims.filter((c) => {
    ...
    return matchesSearch && matchesStatus && matchesTaluka && matchesFromDate && matchesToDate;
  });

  // Reset page when filter inputs change
  React.useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, statusFilter, talukaFilter, fromDate, toDate]);
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-100 text-xs">
          ...
          <div className="flex items-center gap-3">
            ...
            {(searchTerm || statusFilter !== 'All' || talukaFilter || fromDate || toDate) && (
              <button
                onClick={() => {
                  setSearchTerm('');
                  setStatusFilter('All');
