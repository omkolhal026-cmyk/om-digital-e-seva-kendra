...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const menuItems = [
    {
      ...
      roles: ['admin', 'operator', 'sub_agent'],
    },
    {
      id: 'registration',
      label: 'Registration',
  ...
  const filteredMenuItems = menuItems.filter((item) => {
    ...
    if (item.id === 'sub-agent-entries') return true;
    ...
    if (item.id === 'iwbms-checker') return currentUser?.permissions?.canSeeIwbmsChecker !== false;
          ...
          {!isCollapsed ? (
            <div className="p-3 rounded-2xl bg-white/10 backdrop-blur-md border border-white/15 shadow-inner">
              <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-wider text-blue-200/90 mb-1">
                ...
                <span className={`px-2 py-0.5 rounded-full ${isAdmin ? 'bg-amber-400/20 text-amber-300 border border-amber-400/30' : userRole === 'sub_agent' ? 'bg-purple-400/20 text-purple-200 border border-purple-400/30' : 'bg-emerald-400/20 text-emerald-300 border border-emerald-400/30'}`}>
                  {isAdmin ? 'Full Access' : userRole === 'sub_agent' ? 'Sub-Agent' : 'Limited Access'}
                </span>
              </div>
              <div className="text-xs font-bold text-white flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-sky-300" />
                <span>{isAdmin ? 'Administrator Portal' : userRole === 'sub_agent' ? 'Sub-Agent Portal' : 'Operator Workstation'}</span>
              </div>
            </div>
          ) : (
