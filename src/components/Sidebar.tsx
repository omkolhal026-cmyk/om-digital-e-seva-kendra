...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const filteredMenuItems = menuItems.filter((item) => {
    ...
    if (item.id === 'registration') return Boolean(currentUser?.permissions?.canRegister);
    if (item.id === 'renewal') return Boolean(currentUser?.permissions?.canRenew);
    if (item.id === 'claim') return Boolean(currentUser?.permissions?.canClaim);
    return true;
  });
