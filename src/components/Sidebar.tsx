...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const filteredMenuItems = menuItems.filter((item) => {
    ...
    if (item.id === 'registration') return currentUser?.permissions?.canRegister !== false;
    if (item.id === 'renewal') return currentUser?.permissions?.canRenew !== false;
    if (item.id === 'claim') return currentUser?.permissions?.canClaim !== false;
    return true;
  });
