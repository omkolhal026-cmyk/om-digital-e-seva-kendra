...
export async function initMySQL() {
  if (!MYSQL_HOST) {
    ...
    console.log('[Database] No MYSQL_HOST configured in environment. Using local JSON storage backup.');
    return;
  }
  ...
  try {
    ...
    console.log(`[TiDB Cloud / MySQL Database] Connection failed (${connectionError}). Automatically using local JSON storage backup.`);
  }
}
