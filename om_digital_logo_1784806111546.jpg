...
export async function createMySQLRegistration(reg: WorkerRegistration): Promise<WorkerRegistration> {
  const p = getPool();
  await p.query(
    `INSERT INTO registrations (id, mh_number, worker_name, father_name, dob, gender, mobile_number, aadhaar_number, address, village, taluka, district, pincode, bank_name, account_number, ifsc, verification_date, registration_date, operator_name, status, app_status, from_source, next_renewal_date, fee_paid, category, nature_of_work, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
    [
      reg.id,
      reg.mhNumber || '',
  ...
  // Read back from TiDB to confirm insertion
  const [rows]: any = await p.query('SELECT * FROM registrations WHERE id = ?', [reg.id]);
  if (!rows || rows.length === 0) {
    throw new Error(`[TiDB Error] Record insertion read-back failed for registration ID: ${reg.id}`);
  }
  const r = rows[0];
  return {
    id: r.id,
...
export async function updateMySQLRegistration(id: string, reg: Partial<WorkerRegistration>): Promise<WorkerRegistration | null> {
  ...
  if (reg.dob !== undefined) { fields.push('dob = ?'); values.push(reg.dob); }
  ...
  if (reg.verificationDate !== undefined) { fields.push('verification_date = ?'); values.push(reg.verificationDate); }
  if (reg.registrationDate !== undefined) { fields.push('registration_date = ?'); values.push(reg.registrationDate); }
  ...
  if (reg.nextRenewalDate !== undefined) { fields.push('next_renewal_date = ?'); values.push(reg.nextRenewalDate); }
